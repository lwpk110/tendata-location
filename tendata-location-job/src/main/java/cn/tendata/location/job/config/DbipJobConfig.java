package cn.tendata.location.job.config;
...
import cn.tendata.location.job.item.DbipItemProcessor;
import cn.tendata.location.job.item.IplocationItemWriter;
import cn.tendata.location.job.item.SplitItemWriter;
import cn.tendata.location.job.support.GzipBufferedReaderFactory;
import cn.tendata.location.job.support.JobParameters;
...
public class DbipJobConfig {
    ...
    @Bean
    @StepScope
    public FlatFileItemReader<FieldSet> dbipFileReader(@Value("#{jobParameters['" + JobParameters.INPUT_FILE_NAME +
            "']}")
                                                               String pathToFile) {
